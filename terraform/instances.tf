...
locals {
  ...
  bio_sup_service_content = templatefile(
    "${path.module}/templates/bio-sup.service.tpl",
    {
  ...
  bio_sup_init_content = templatefile(
    "${path.module}/templates/bio-sup.init.tpl",
    {
      flags            = "--auto-update --peer ${join(" ", var.peers)} --channel ${var.sup_release_channel} --listen-gossip 0.0.0.0:${var.gossip_listen_port} --listen-http 0.0.0.0:${var.http_listen_port}"
      log_level        = var.log_level
      enabled_features = var.enabled_features
...
resource "aws_instance" "api" {
  ...
  provisioner "file" {
    content = local.bio_sup_service_content
...
resource "aws_instance" "linux2-worker" {
  ...
  provisioner "file" {
    content = local.bio_sup_init_content
