...
const pubFile = fs.readFileSync(
  __dirname + `/../fixtures/deletemeifyoucan-${revision}.pub`,
  "utf8"
);
const secretFile = fs.readFileSync(
  __dirname + `/../fixtures/deletemeifyoucan-${revision}.sig.key`,
  "utf8"
);
...
describe("Origin API", function () {
    ...
    it("returns the origin", function (done) {
      request
        ...
        .end(function (err, res) {
          ...
          expect(res.body.default_package_visibility).to.equal(
            global.originNeurosis.default_package_visibility
          );
  ...
  describe("Origin secret creation", function () {
    it("requires authentication", function (done) {
      request
        .post("/depot/origins/neurosis/secret")
  ...
  describe("Origin secret list", function () {
    it("requires authentication", function (done) {
      request
        .get("/depot/origins/neurosis/secret")
        .expect(401)
        .end(function (err, res) {
          expect(res.text).to.be.empty;
          done(err);
  ...
  describe("Origin secret deletion", function () {
    it("requires authentication", function (done) {
      request
        .delete("/depot/origins/neurosis/secret/foo")
        .expect(401)
        .end(function (err, res) {
          expect(res.text).to.be.empty;
          done(err);
...
describe("Related Origin API functions", function () {
  describe("Invite prereq for ownership transfer", function () {
    it("returns the invitation", function (done) {
      request
        .post("/depot/origins/umbrella/users/bobo/invitations")
        .set("Authorization", global.weskerBearer)
        .expect(201)
        .end(function (err, res) {
  ...
  describe("Origin Transfer", function () {
    it("requires authentication", function (done) {
      request
        .post("/depot/origins/umbrella/transfer/bobo")
        .expect(401)
        .end(function (err, res) {
          expect(res.text).to.be.empty;
          done(err);
    ...
    it("cannot be transferred from a user to themselves", function (done) {
      request
        .post("/depot/origins/umbrella/transfer/wesker")
        .set("Authorization", global.weskerBearer)
        .expect(422)
        .end(function (err, res) {
    ...
    it("recipient must already be a member of the origin", function (done) {
      request
        .post("/depot/origins/umbrella/transfer/mystique")
        .set("Authorization", global.weskerBearer)
        .expect(403)
        .end(function (err, res) {
          expect(res.text).to.be.empty;
    ...
    it("no longer returns owner for role query", function (done) {
      request
        .get("/depot/origins/umbrella/users/wesker/role")
        .type("application/json")
        .accept("application/json")
        .set("Authorization", global.boboBearer)
        .expect(200)
    ...
    it("returns correct owner for role query", function (done) {
      request
        .get("/depot/origins/umbrella/users/bobo/role")
        .type("application/json")
        .accept("application/json")
        .set("Authorization", global.boboBearer)
        .expect(200)
  ...
  describe("Origin Departure", function () {
    it("requires authentication", function (done) {
      request
        .post("/depot/origins/umbrella/depart")
        .expect(401)
        .end(function (err, res) {
          expect(res.text).to.be.empty;
          done(err);
    ...
    it("must be an origin that exists", function (done) {
      request
        .post("/depot/origins/grifoobity/depart")
        .set("Authorization", global.mystiqueBearer)
        .expect(422)
        .end(function (err, res) {
          done(err);
    ...
    it("no longer shows the departed user as a member", function (done) {
      request
        .get("/depot/origins/umbrella/users")
        .type("application/json")
        .accept("application/json")
        .set("Authorization", global.boboBearer)
        .expect(200)
  ...
  describe("Origin deletion - extended", function () {
    ...
    describe("[Prereq 2]: create an origin and add a member", function () {
      ...
      it("invites a member", function (done) {
        request
          .post("/depot/origins/deletemeifyoucan/users/wesker/invitations")
          .set("Authorization", global.boboBearer)
          .expect(201)
          .end(function (err, res) {
    ...
    describe("2. Attempt to delete an origin when an additional member exists", function () {
      it("gets 409 before the non-owner member is removed", function (done) {
        request
          ...
          .end(function (err, res) {
            expect(res.text).to.match(
              /^There are 2 members remaining in origin deletemeifyoucan./
            );
            done(err);
          });
      });
      it("departs the non-owner member", function (done) {
        request
          .post("/depot/origins/deletemeifyoucan/depart")
          .set("Authorization", global.weskerBearer)
          .expect(204)
          .end(function (err, res) {
            expect(res.text).to.be.empty;
    ...
    describe("[Prereq 3]: create an origin and add an integration", function () {
      ...
      it("adds an integration", function (done) {
        request
          .put(
            "/depot/origins/deletemeifyoucan/integrations/docker/testintegration"
          )
          .type("application/json")
          .accept("application/json")
          .set("Authorization", global.boboBearer)
    ...
    describe("3. Attempt to delete an origin when an integration exists", function () {
      it("gets 409 before the integration is removed", function (done) {
        request
          ...
          .end(function (err, res) {
            expect(res.text).to.match(
              /^There are 1 integrations remaining in origin deletemeifyoucan./
            );
            done(err);
          });
      });
    ...
    describe("[Prereq 4]: create an origin and add a channel", function () {
      ...
      it("creates a channel", function (done) {
        request
          .post("/depot/channels/deletemeifyoucan/testchan")
          .set("Authorization", global.boboBearer)
          .expect(201)
          .end(function (err, res) {
    ...
    describe("4. Attempt to delete an origin when channel exists", function () {
      it("gets 409 before the channel is removed", function (done) {
        request
          ...
          .end(function (err, res) {
            expect(res.text).to.match(
              /^There are 3 channels remaining in origin deletemeifyoucan. Only two are allowed \[unstable, stable\]/
            );
            done(err);
          });
      });
      it("deletes the channel", function (done) {
        request
          .delete("/depot/channels/deletemeifyoucan/testchan")
          .set("Authorization", global.boboBearer)
          .expect(200)
          .end(function (err, res) {
            expect(res.text).to.be.empty;
    ...
    describe("[Prereq 5]: create an origin and upload a package", function () {
      ...
      it("uploads the pub signing key", function (done) {
        request
          .post(`/depot/origins/deletemeifyoucan/keys/${revision}`)
          .set("Authorization", global.boboBearer)
          .send(pubFile)
          .expect(201)
          .end(function (err, res) {
            expect(res.text).to.equal(
              `/origins/deletemeifyoucan/keys/${revision}`
            );
            done(err);
          });
      });
      it("uploads the package", function (done) {
        request
          .post(`/depot/pkgs/deletemeifyoucan/testapp/0.1.3/${release1}`)
          .set("Authorization", global.boboBearer)
          .set("Content-Length", file1.length)
          .query({
            checksum:
              "11e7e19b9349f0e92fe454a5368e3e50422604a509e821bf538284aad0c984c1",
          })
          .send(file1)
          .expect(201)
          .end(function (err, res) {
            expect(res.text).to.equal(
              `/pkgs/deletemeifyoucan/testapp/0.1.3/${release1}/download`
            );
            done(err);
          });
      });
    ...
    describe("5. Attempt to delete an origin when a package exists", function () {
      it("gets 409 before the package is removed", function (done) {
        request
          ...
          .end(function (err, res) {
            expect(res.text).to.match(
              /^There are 1 packages remaining in origin deletemeifyoucan./
            );
            done(err);
          });
      });
      it("rejects deletion of package settings while package exists", function (done) {
        request
          .delete("/settings/deletemeifyoucan/testapp")
          .set("Authorization", global.boboBearer)
          .expect(409)
          .end(function (err, res) {
            done(err);
      ...
      it("deletes the package", function (done) {
        request
          .delete(`/depot/pkgs/deletemeifyoucan/testapp/0.1.3/${release1}`)
          .set("Authorization", global.boboBearer)
          .expect(204)
          .end(function (err, res) {
            expect(res.text).to.be.empty;
      ...
      it("rejects deletion of origin while package settings exists", function (done) {
        request
          .delete("/depot/origins/deletemeifyoucan")
          .set("Authorization", global.boboBearer)
          .expect(409)
      ...
      it("deletes the package settings", function (done) {
        request
          .delete(`/settings/deletemeifyoucan/testapp`)
          .set("Authorization", global.boboBearer)
          .expect(204)
      ...
      it("succeeds after the package setting was deleted", function (done) {
        request
          .delete("/depot/origins/deletemeifyoucan")
          .set("Authorization", global.boboBearer)
          .expect(204)
