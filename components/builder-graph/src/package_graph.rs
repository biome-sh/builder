...
use biome_builder_db::models::package::PackageWithVersionArray;

// use biome_builder_protocol as protocol;

use crate::{bio_core::{error as herror,
...
impl PackageInfo {
    ...
    pub fn extract_plan_deps(&mut self, verbose: bool) {
        ...
        if !(found_deps && found_bdeps) {
            ...
            // 2) User packages statically built using system dependencies. While not 'pure' biome
...
mod test {
    ...
    fn disallow_circular_dependency() {
        ...
        let package1 =
            mk_package_with_versionarray("foo/bar/1/2", "x86_64-linux", &["foo/baz/1/2"], &empty);
        packages.push(package1);
        let package2 =
            mk_package_with_versionarray("foo/baz/1/2", "x86_64-linux", &["foo/bar/1/2"], &empty);
    ...
    fn pre_check_with_dep_not_present() {
        ...
        let package2 =
            mk_package_with_versionarray("foo/baz/1/2", "x86_64-linux", &["foo/xyz/1/2"], &empty);
    ...
    fn mk_package_with_versionarray(ident: &str,
                                    target: &str,
                                    rdeps: &[&str],
                                    bdeps: &[&str])
                                    -> PackageWithVersionArray {
    ...
    fn mk_builder_package_ident_vec(vals: &[&str]) -> Vec<BuilderPackageIdent> {
        vals.iter()
