...
async fn handle_push(req: &HttpRequest, body: &str) -> HttpResponse {
    ...
    debug!("Received GitHub web hook; sender={}, repository={} repository_id={} ref={} \
            installation_id={}",
           hook.sender.login,
                ...
                .map_err(Error::Github)
    {
        Ok(Some(contents)) => {
            ...
            match contents.decode().map_err(Error::Github) {
                Ok(ref bytes) => {
                    match BuildCfg::from_slice(bytes).map_err(Error::BuilderCore) {
                        Ok(cfg) => Ok(cfg),
                        Err(err) => {
        ...
        Ok(None) => {
            debug!("No bldr.toml found for repo {}, using default config",
                   hook.repository.full_name);
            Ok(BuildCfg::default())
        }
        Err(err) => {
...
async fn read_plan(github: &GitHubClient,
                ...
                .map_err(Error::Github)
    {
        Ok(Some(contents)) => {
            match contents.decode().map_err(Error::Github) {
                Ok(bytes) => {
                    match Plan::from_bytes(bytes.as_slice()).map_err(Error::BiomeCore) {
...
mod test {
    ...
    use crate::{config::Config,
                ...
                bio_core::package::{target::PackageTarget,
                                    Plan},
    ...
    fn valid_plan_is_not_rejected() {
        ...
        let repo_url = "https://github.com/biome-sh/testapp.git";
        ...
        assert!(!should_reject_build(&plan,
                                     &project,
                                     &config,
                                     "https://github.com/biome-sh/testapp.git"));
    }
    ...
    fn project_with_invalid_vcs_data_is_rejected() {
        ...
        assert!(should_reject_build(&plan,
                                    &project,
                                    &config,
                                    "https://github.com/biome-sh/testapp.gut"));
    }
}
