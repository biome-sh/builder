...
mod test {
    ...
    use chrono::{DateTime,
                 Duration,
                 Utc};
    use biome_builder_db::datastore_test;
    use biome_builder_protocol::message::{jobsrv::*,
    ...
    fn create_job() {
        ...
        let raw_job = biome_builder_db::models::jobs::Job::get(job_id as _, &conn).unwrap();
    ...
    fn next_pending_job() {
        ...
        let conn = ds.get_pool().get_conn().unwrap();
        let list_project_jobs = biome_builder_db::models::jobs::ListProjectJobs {
            name: helpers::PROJECT_NAME.to_string(),
            page: 1,
            limit: 100,
        };
        let (_, job_count) =
            biome_builder_db::models::jobs::Job::list(list_project_jobs, &conn).unwrap();
    ...
    fn create_audit_entry() {
        ...
        let conn = ds.get_pool().get_conn().unwrap();
        let raw_audit_group =
            biome_builder_db::models::jobs::AuditJob::get_for_group(grog_id, &conn).unwrap();
    ...
    fn set_job_group_project_state() {
        ...
        let conn = ds.get_pool().get_conn().unwrap();
        let before =
            biome_builder_db::models::jobs::GroupProject::get_group_projects(group_id.try_into()
                                                                                       .unwrap(),
                                                                               &conn).unwrap();
    ...
    fn sync_jobs() {
        ...
        let raw_job = biome_builder_db::models::jobs::Job::get(job_id, &conn).unwrap();
