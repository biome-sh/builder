...
echo "This script will take all of the data that's currently in builder_originsrv, builder_sessionsrv and builder_jobsrv"
...
if [ -n "${MIGRATE_JOBSRV:-}" ]; then
    ...
    jobsrv_tables=(groups group_projects jobs busy_workers)
    for table in "${jobsrv_tables[@]}"; do
        echo "Copying ${table} from builder_jobsrv to builder"
        execute_sql builder_jobsrv "\\copy ${table} to stdout" | execute_sql builder "\\copy ${table} from stdin"
    done
fi
